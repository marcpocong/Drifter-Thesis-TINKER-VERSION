...
class Phase5LauncherAndDocsSyncTests(unittest.TestCase):
    ...
    def test_phase5_service_generates_outputs_and_handles_missing_optional_artifacts(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("first-code search box", prototype_2016["description"].lower())
            self.assertIn("west coast of the philippines", prototype_2016["description"].lower())
            self.assertIn("operative scientific/display extents", prototype_2016["notes"].lower())
