...
- the active thesis-facing Mindoro-focused provenance box is kept separate at `118.751-124.305E / 10.620-16.026N`, and the publication package now includes a shared study-box reference figure plus per-box geography panels so that focused box, `mindoro_case_domain`, the scoring-grid display bounds, and the prototype_2016 first-code search box are not collapsed into one extent
